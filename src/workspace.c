...
static void assume_handler(struct wl_list * handlers, wlkit_workspace_handler_t handler) {
	struct wlkit_workspace_handler * wrapper = malloc(sizeof(*wrapper));
	wrapper->handler = handler;
	wl_list_insert(handlers, &wrapper->link);
};

void wlkit_on_workspace_create(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.create, handler);
}

void wlkit_on_workspace_destroy(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.destroy, handler);
}

void wlkit_on_workspace_activate(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.activate, handler);
}

void wlkit_on_workspace_deactivate(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.deactivate, handler);
}

void wlkit_on_workspace_window_added(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.window_added, handler);
}

void wlkit_on_workspace_window_removed(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.window_removed, handler);
}

void wlkit_on_workspace_layout_change(struct wlkit_workspace * workspace, wlkit_workspace_handler_t handler) {
	assume_handler(&workspace->handlers.layout_change, handler);
}
